...
def bestToTarget(cardSet: Multiset, target, isMostPlays=False):  # unlimited cards only
    ...
    for v, _ in cardSet.items.items():
        newValues = cardSet
        pre = bestToTarget(newValues, target - v, isMostPlays)
        res = best(res, pre + 1)
...
def bottumUp(cardSet: Multiset, target, isMostPlays=False):  # limited cards only
    result = [MIN] * (target+1) if isMostPlays else [MAX] * (target+1)
    ...
    for tempTarget in range(1, target + 1):
        for v, _ in cardSet.items.items():
            if v > tempTarget or valuesAvailable[tempTarget - v].numberOf(v) == 0:
                continue
            if isMostPlays and result[tempTarget - v] + 1 > result[tempTarget] or not isMostPlays and result[tempTarget - v] + 1 < result[tempTarget]:
