...
public class Main {
    ...
    static final char BASECHAR = ('a' - 1); // whatever comes before 'a' (helps sort "a" before "aa" and identifies cycles)
    ...
    public static int findMaxLCP(String s, int[] sa) {
        ...
        int commonChars = 0;
        //Start from the longest suffix x. Compare it to the element that comes before x in sa
        for (int i = 0; i < sa.length; i++) {
            if (saIndexOf[i] == 0) {
                commonChars = 0;
                continue;
